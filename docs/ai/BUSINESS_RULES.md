...
- **`super_admin`** — every permission that exists, granted automatically (the seed loops over all seeded permissions and grants each to `super_admin`), so any future migration/seed adding a new permission key automatically flows through to `super_admin` too.

**Only these two roles are actually seeded.** The README and `docs/DECISIONS.md` describe a richer intended role model (Admin, Product Manager, Inventory Manager, Order Manager, Customer Support, Finance, Content Manager) as the target design, but no seed data, migration, or code anywhere creates those additional roles today — `grep`ing the codebase for role names beyond `customer`/`super_admin` returns nothing. This is a real, verified discrepancy between the aspirational docs and the current database state — see `KNOWN_LIMITATIONS.md`. Until those roles are actually seeded and granted a curated permission subset, every admin capability in this app is only reachable by a `super_admin` account (see `scripts/bootstrap-super-admin.ts` / `npm run bootstrap:admin`).

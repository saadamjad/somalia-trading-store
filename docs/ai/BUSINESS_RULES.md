...
## Duplicate-checkout prevention (idempotency)

`order-service.ts`'s `persistOrder` guards against a double-click (or any retried request) creating two orders, via two different mechanisms depending on the path:

- **Authenticated cart checkout** (`clearCart: true`): the cart's own row count is the atomic gate — the cart is cleared *first*, inside the transaction, and its returned `{count}` tells you whether there was anything to check out; a second concurrent request sees an already-empty cart and is rejected.
- **Guest checkout and quote-to-order conversion** (`clearCart: false`, no server cart to gate on): a `CheckoutLock` row, keyed `(userId, key)`, claimed atomically via `INSERT ... ON CONFLICT (userId, key) DO NOTHING RETURNING id` (`checkout-lock-repository.ts`). `key` is a **client-generated idempotency key** — the checkout page generates one random key per page load and resends it on every "Place Order" click; a genuine double-click sends the same key twice (rejected, `DuplicateCheckoutError` → HTTP 409), while a fresh checkout (a real second purchase) generates a new key (allowed). Quote conversion uses the quote's own id as its key (`quote-conversion:${quote.id}`) — re-converting the same quote is always a duplicate. A caller with no client-supplied key falls back to a deterministic content hash of the request (`deriveFallbackIdempotencyKey`), which still catches exact resubmissions but can't distinguish two genuinely-repeated legitimate purchases from each other.

**Why not a time window?** An earlier version of this fix used a flat time-window debounce instead of a key, and it was wrong — it broke a legitimate test where the same guest places two genuinely separate orders moments apart, because a time window cannot tell that apart from a duplicate: the two requests are byte-identical. The key-based design resolves the ambiguity correctly regardless of timing or content.
...
- **`staff`** — operational permissions only (products, categories, inventory, orders, CMS, reviews). Explicitly excluded from anything that surfaces financial figures or admin-account management: no `dashboard.view`, `reports.view`, `refunds.*`, `quotes.*`, `coupons.*`, or `admin_users.*` (see `staffExcludedKeys` in `prisma/seed.ts`). Note the deliberate split within this build-out's two new permission pairs: `staff` **can** moderate reviews (`reviews.*` — content moderation, not financial) but **cannot** manage coupons (`coupons.*` — discounts affect order totals, financial).
...
Example seeded permission keys (`prisma/seed.ts` `permissionKeys`): `products.view`, `products.create`, `products.update`, `products.delete`, `categories.create`, `categories.update`, `categories.delete`, `inventory.view`, `inventory.update`, `orders.view`, `orders.update`, `customers.view`, `customers.update`, `refunds.view`, `refunds.manage`, `quotes.view`, `quotes.manage`, `cms.view`, `cms.manage`, `reports.view`, `reviews.view`, `reviews.manage`, `coupons.view`, `coupons.manage`.

...
## Database indexes (Phase 17, extended by the reviews/coupons/variants build-out)

53 `@@index` declarations across `prisma/schema.prisma` (up from 41 at Phase 17 — the reviews/coupons/variants/checkout-lock build-out added indexes for `Review` (`[productId, status]`, `[userId]`), `Coupon` (`[active]`), `CouponRedemption` (`[couponId, userId]`), `ProductVariant`/`VariantInventory`/`VariantInventoryTransaction`, and `CartItem`/`OrderItem`'s new `variantId` columns, following the same principle below). Added by auditing actual query patterns, not speculatively. The most consequential set is on `Order` — `userId`, `orderNumber`, `status`, `paymentStatus`, `createdAt` — because `Order` is the highest-write-volume table (one row per checkout, growing forever, unlike small bounded tables like `Category`/`Role`), and `status`/`paymentStatus` are both used as standalone filters in `orderRepository.buildAdminWhere` and `dashboardRepository.orderStatusCounts`'s groupBy, while `createdAt` is the admin list's default sort column and the date-range filter used across `orderRepository.adminFindMany`, the dashboard service, and `reportRepository`. Every other model's indexes follow the same principle: indexed where an actual filter/sort/join pattern in a repository uses that column, not by default.

## Duplicate-submission / idempotency protection

`CheckoutLock` (see `BUSINESS_RULES.md` "Duplicate-checkout prevention") guards guest checkout and quote-to-order conversion against a double-click or retried request creating two orders, using a client-generated idempotency key claimed via atomic `INSERT ... ON CONFLICT DO NOTHING` — the same class of concern rate limiting addresses (abusive/duplicate request patterns) but solved with a different mechanism, since rate limiting alone can't distinguish "the same click retried" from "a new, valid checkout" and would either be too strict (block legitimate rapid separate purchases) or too loose (allow the duplicate through if it's the first request within the window). See `docs/DECISIONS.md` D-017.
